package com.upskill.java_1;

public class AccessModifier {

	public String name = "Pavel";         // public - Any class can access
	
	private int age = 25;                 // private - Only same class can access
	
	protected int ssn = 12345645;         // protected - classes in the same package and subclass
	
	String address = "Heritage Tower";    // default - classes in same package can access
	
	

		// public - Any class can access
		
		// private - Only same class can access
		
		// protected - classes in the same package and subclass can access
		
		// default - classes in same package can access
		
		
}
